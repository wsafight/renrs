...
const args = process.argv.slice(2),
  option = (name, fallback) => (args.includes(name) ? args[args.indexOf(name) + 1] : fallback);
const port = Number(option('--port', '4185'));
const statePath = path.resolve(
  option('--state', process.env.RENRS_LAUNCHER_STATE || path.join(root, '.renrs/launcher.json')),
);
...
function binary(name, sdk = state.sdk) {
  return path.join(
    sdk,
    existsSync(path.join(sdk, 'bin')) ? 'bin' : '',
...
async function sdkStatus(sdk = state.sdk) {
  ...
  for (const name of [
    ...
    tools[name] = existsSync(binary(name, sdk));
  ...
  return new Promise((resolve, reject) => {
    ...
    const append = (current, chunk) => {
      ...
      if (Buffer.byteLength(next) > 4 * 1024 * 1024) {
        child.kill('SIGTERM');
        throw new Error('Machine report exceeds 4 MiB');
      }
      return next;
    };
    ...
    child.once(
      ...
      finish(() => {
        try {
          ...
          if (report.protocol_version !== 1) throw new Error('Unsupported machine protocol');
          resolve(report);
        } catch (error) {
    ...
    timer = setTimeout(() => {
      child.kill('SIGTERM');
      finish(reject)(new Error('Machine inspection timed out'));
    }, timeout);
  });
}
function project(id) {
...
async function resource(project, name) {
  const base = await realpath(project.path),
    target = await realpath(path.join(base, name));
...
async function scripts(directory, prefix = '', depth = 0) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    if (entry.name.startsWith('.') || ['dist', 'node_modules', 'target'].includes(entry.name))
      continue;
    if (entry.isDirectory())
      files.push(
        ...(await scripts(path.join(directory, entry.name), `${prefix}${entry.name}/`, depth + 1)),
      );
...
async function body(request) {
  ...
  for await (const chunk of request) {
    body += chunk;
    if (Buffer.byteLength(body) > 1024 * 1024) throw new Error('Request exceeds 1 MiB');
  }
  return JSON.parse(body || '{}');
}
async function register(directory) {
  const location = await realpath(directory);
  if (!(await stat(location)).isDirectory() || !(await scripts(location)).length)
    throw new Error('Directory has no .rns scripts');
  let item = state.projects.find((item) => item.path === location);
  if (!item) {
    item = { id: randomUUID(), path: location, name: path.basename(location) };
...
async function api(request, url) {
  if (request.method === 'GET') {
    if (url.pathname === '/api/state')
      return { ...state, sdk: await sdkStatus(), jobs: jobs.list() };
    ...
    if (url.pathname === '/api/script') {
      ...
      if (!file.endsWith('.rns')) throw new Error('Expected .rns');
      const text = await readFile(file, 'utf8');
      return { text, revision: digest(text) };
    }
    if (url.pathname === '/api/inspection') {
      const item = project(url.searchParams.get('id'));
      return machine(binary('renrs-inspect'), [item.path]);
    }
    if (url.pathname === '/api/migration-report') {
      ...
      if ((await stat(file)).size > 4 * 1024 * 1024)
        throw new Error('Migration report exceeds 4 MiB');
  ...
  switch (url.pathname) {
    case '/api/register':
      return register(data.path);
    case '/api/remove':
      state.projects = state.projects.filter((item) => item.id !== data.id);
      await persist();
      return {};
    case '/api/sdk': {
      const sdk = await realpath(data.path);
      const status = await sdkStatus(sdk);
      if (!status.ready) throw new Error('SDK is missing required executables');
      state.sdk = sdk;
      await persist();
      return status;
    }
    ...
    case '/api/migrate': {
      const source = await realpath(data.source),
        destination = path.resolve(data.output);
      if (existsSync(destination)) throw new Error('Migration destination already exists');
      const job = jobs.start(
        "Migrate Ren'Py project",
        binary('renrs-migrate'),
        [source, destination],
        root,
        600000,
        () => register(destination),
    ...
    case '/api/impact': {
      const baseline = project(data.baseline),
    ...
    case '/api/script': {
      const file = await resource(project(data.id), data.file);
      if (!file.endsWith('.rns') || typeof data.text !== 'string')
        throw new Error('Expected script');
      if (digest(await readFile(file, 'utf8')) !== data.revision)
    ...
    case '/api/task': {
      ...
      const commands = {
        check: ['renrs-check', [item.path]],
        run: ['renrs', [item.path]],
        graph: ['renrs-graph', [item.path]],
        pack: ['renrs-pack', [item.path, output]],
        build: ['renrs-build', [item.path, output]],
        web: [
          'renrs-web-build',
          [
            item.path,
            output,
            '--shell',
            existsSync(path.join(state.sdk, 'web'))
              ? path.join(state.sdk, 'web')
              : path.join(root, 'web/dist'),
          ],
        ],
      };
      const command = commands[data.action];
      if (!command || command[1].some((value) => !value))
        throw new Error('Invalid task or missing output directory');
      if (['pack', 'build', 'web'].includes(data.action) && existsSync(output))
        throw new Error('Output already exists');
      return jobs.public(
        jobs.start(
          `${item.name}: ${data.action}`,
          binary(command[0]),
          command[1],
          root,
          data.action === 'run' ? 0 : 600000,
        ),
      );
    }
    default:
...
const launcherRoot = path.join(root, 'launcher', 'dist');
const types = {
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      const result =
        request.method === 'POST'
          ? await (mutations = mutations.catch(() => {}).then(() => api(request, url)))
    ...
    if (url.pathname === '/thumbnail')
      file = await resource(project(url.searchParams.get('id')), 'images/studio.png');
    else {
      const relative = decodeURIComponent(
        url.pathname === '/' ? 'index.html' : url.pathname.slice(1),
      );
      file = path.resolve(launcherRoot, relative);
      if (!file.startsWith(`${launcherRoot}${path.sep}`) || !(await stat(file)).isFile())
...
server.listen(port, '127.0.0.1', () =>
  console.log(`RenRS Launcher: http://127.0.0.1:${server.address().port}`),
);
