...
export class Jobs {
  ...
  start(name, executable, args, cwd, timeout = 600000, afterSuccess) {
    ...
    this.jobs.length = Math.min(30, this.jobs.length);
    const append = (chunk) => {
      job.log = (job.log + chunk.toString()).slice(-131072);
    };
  ...
  cancel(id) {
    ...
    if (!job || job.code !== null) return;
    job.child.kill('SIGTERM');
    const timer = setTimeout(() => {
      if (job.code === null) job.child.kill('SIGKILL');
    }, 2000);
    timer.unref();
  }
  stop() {
  ...
  public(job) {
    return { id: job.id, name: job.name, log: job.log, code: job.code, started: job.started };
  }
  list() {
