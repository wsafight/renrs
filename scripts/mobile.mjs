...
if (!source || !destination || extra.length)
  throw new Error('Usage: node scripts/mobile.mjs <web-build> <new-mobile-directory>');
const input = path.resolve(source),
  output = path.resolve(destination);
if (output === input || output.startsWith(`${input}${path.sep}`))
  throw new Error('Mobile output must be outside the Web build');
...
if (!/^[a-zA-Z][\w]*(\.[a-zA-Z][\w]*)+$/.test(id))
  throw new Error('Mobile builds need a reverse-domain project id, such as org.studio.game');
await mkdir(output); // Refuse to replace an existing project or signing configuration.
await cp(input, path.join(output, 'www'), {
  recursive: true,
  filter: async (file) => {
