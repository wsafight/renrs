...
本文是按任务查询的语法手册，不要求从头读到尾。第一次运行项目请先完成
[快速开始](QUICKSTART.md)；遇到具体创作需求时，可从下表直接进入对应章节。

| 要做的事 | 章节 |
| --- | --- |
| 组织多文件项目和声明角色、图片、变量 | [项目结构](#项目结构)、[配置与声明](#配置与声明) |
| 写对白、富文本、NVL 和分页 | [对白、插值与文字标签](#对白插值与文字标签) |
| 控制立绘、动画、转场和视频 | [舞台与 transform](#舞台与-transform) |
| 编写变量、条件、菜单和可复用标签 | [变量与表达式](#变量与表达式)、[条件与菜单](#条件与菜单) |
| 配置音频、本地化、主题和字体 | [音频与暂停](#音频与暂停)、[稳定 ID 与本地化](#稳定-id-与本地化) |
| 排查脚本问题和热重载行为 | [检查与热重载](#检查与热重载) |
...
Velin parser 产生带源码 span 的节点；RenRS 在完成静态检查后递归移除 span，保持已发布
程序 JSON 的表达式结构。字符串中的方括号会先替换成等长且不与输入冲突的临时字符，再交给
Velin 解析并原样还原，因此错误列号和 64 KiB 表达式长度上限不会因兼容处理发生偏移。

静态检查使用空类型环境，只拒绝从表达式自身就能证明的错误；动态剧情变量不会因为加载时类型
未知而被误报。`random` 和 `chance` 需要显式 RNG 状态，而当前存档、回滚和热重载没有该
状态契约，所以源码入口明确拒绝这两个函数；手工构造 AST 也会在无 RNG 的求值入口失败。只有在
种子、推进、存档、回滚及热重载语义一起定义后，才可安全开放。
