...
Launcher 是本地项目工作区，复用 Rust 命令行工具完成迁移、检查、影响分析、运行、剧情图输出和构建。
它提供项目列表、脚本编辑、质量报告、任务日志及 SDK 路径配置。
...
“迁移 Ren'Py 项目”选择源 `game` 目录或单个 `.rpy` 及新的输出目录。任务完成后自动注册输出；
质量视图按稳定 code 汇总迁移问题，并保留完整 `migration-report.json` 供编辑器定位。

脚本视图读取 `.rns` 文件。保存会检查载入时的文件指纹：文件被外部编辑后，必须重新载入，
避免覆盖别处的修改。保存成功后启动项目检查。

“质量”视图调用 `renrs-inspect`，展示路线指令覆盖率、路线通过数、静态不可达标签、变量、翻译缺失、
源码诊断和逐路线结果。静态可达与实际路线覆盖分别显示；刷新按钮重新读取当前磁盘内容。
选择另一个已注册项目作为基线后，可直接查看标签、路线、结局和存档结构的影响摘要。
