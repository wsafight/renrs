...
function renderQuality(envelope){
  ...
  $('diagnostic-list').replaceChildren();$('uncovered-list').replaceChildren();$('route-list').replaceChildren();$('locale-list').replaceChildren();
...
async function loadQuality(force=false){if(!selected)return;if(!force&&qualityProject===selected)return;qualityProject=selected;$('quality-status').textContent='正在检查项目…';try{renderQuality(await api(`inspection?id=${encodeURIComponent(selected)}`));}catch(error){qualityProject=undefined;throw error;}}
function renderJobs(){
