...
fn run() -> Result<(), String> {
    ...
    match args.as_slice() {
        [command, directory] if command == "generate-reference" => {
            renrs::benchmark::generate_reference(Path::new(directory))?;
            println!("Generated first-party reference project at {directory}");
            Ok(())
        }
        ...
        _ => Err("usage: renrs-bench <project> [iterations] | renrs-bench generate <new-directory> <chapters> <lines-per-chapter> | renrs-bench generate-reference <new-directory>".to_owned()),
    }
}
