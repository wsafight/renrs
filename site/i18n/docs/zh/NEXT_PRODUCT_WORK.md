# 产品后续开发

范围：按优先级补齐剩余产品和性能缺口。商业项目验收、跨平台验收和上架由所有者排除。
旧存档格式不必保持兼容。

- [x] P0：共享快照编码，以及轻量原生/Web 存档列表。
- [x] P1：可组合界面、滚动容器、数据驱动控件和拖放。
- [x] P1：镜头、运行时角色层和更丰富的呈现。
- [x] P1：带持久化/回滚语义的确定性扩展 API。
- [x] P1：复杂文本排版、回退字体和本地化规则。
- [ ] P2：项目 Launcher、SDK 流程、模板和作者文档（已实现；端到端验证待完成）。
- [ ] P2：媒体配置、图形扩展和平台服务适配（新工作待开始）。

每一项都要先实现再做针对性验证才能勾选。外部 SDK 和服务在不可用时必须明确标出。

Launcher 用 `node scripts/launcher.mjs` 启动；SDK 包用
`node scripts/package-sdk.mjs <new-directory> <binary-directory>` 组装。
Launcher 把 check、run、graph、pack 和 build 交给现有 Rust 工具，日志有界，
能发现过期脚本编辑，并且只存储本地路径。主题文件支持回退字体族。独立的音乐、
音效、语音音量默认值和本地平台适配早于本轮，它们并不完成计划中的媒体、图形或
平台服务扩展。Live2D、自定义 shader 和云/商店服务仍未实现。商业项目和跨平台
验收不在本开发范围内。

## 存档证据

快照 v5 对标量/集合节点做 intern，并恢复共享集合。存档容器仍是 v2，只接受当前
快照。原生摘要供两套列表 API 使用，并在轮换时复制；Web IndexedDB 摘要与载荷分离，
按项目索引。加载仍会检查载荷。

同一份 818 指令/401 次交互的大状态夹具，release 构建，本地各跑一次前后对比：
快照 34,773,179 -> 348,528 字节；前六个存档列表 3274.05 -> 0.66 ms；保存
197.13 -> 23.20 ms。新的完整磁盘加载/恢复测量为 3.89 ms。这些是特定工作负载的
观察，不是跨引擎结论。

共享快照/回滚测试、原生存档测试，以及 1280/390px 的 Web 数据和存档流程均通过。
界面原生裁剪和分层立绘验证仍在进行。
