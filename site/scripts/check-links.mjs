...
async function resolveFile(pathname, from) {
  ...
  try {
    if ((await stat(absolute)).isDirectory() && await fileExists(path.join(absolute, 'index.html'))) {
...
for (const file of files) {
  ...
  for (const node of [...$('a[href], img[src], link[href]:not([rel="canonical"]), script[src]')]) {
    ...
    if (hash && target.endsWith('.html')) {
      ...
      if (!ids.has(decodeURIComponent(hash))) errors.push(`${path.relative(dist, file)} -> ${url}`);
    }
  }
}
