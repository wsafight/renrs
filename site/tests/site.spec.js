...
test('homepage presents the product and navigation', async ({page}, info) => {
  ...
  await expect(page.getByRole('heading', {level: 1, name: 'Guides and development records'})).toBeVisible();
});
...
test('docs catalog, table of contents and adjacent pages', async ({page}, info) => {
  ...
  await page.getByRole('navigation', {name: 'Adjacent pages'}).getByRole('link', {name: /Workspace/}).click();
...
test('catalog pages render their titles', async ({page}) => {
  const sample = pages.filter(item => ['start/quickstart', 'guides/scripting', 'reference/cli', 'project/status'].includes(item.slug));
