...
for (const width of [1280, 390]) {
  test(`nested viewports and persistent data controls at ${width}`, async ({ page }, info) => {
    ...
    if (width === 1280) {
      ...
      await page
        .getByRole('button', { name: 'Pack selected item', exact: true })
        .dragTo(page.getByRole('button', { name: 'Travel bag', exact: true }));
    ...
    await expect(page.locator('#modal')).toContainText(
      width === 1280 ? 'Packed: Radio' : 'Packed: Map',
    );
    await expect(page.locator('#modal')).toContainText('Reward: 1');
