...
for (const width of [1280, 390]) {
  test(`custom screens, preferences and portable saves at ${width}`, async ({page}, info) => {
    ...
    await page.locator('[data-screen=main_menu]').getByRole('button', {name: 'New game', exact: true}).click();
    await expect(page.locator('.custom-dialogue strong').filter({hasText: 'receiver'})).toBeVisible();
    await page.getByLabel('Name', {exact: true}).fill('Alex');
    await page.locator('[data-screen=dialogue]').getByRole('button', {name: 'Save', exact: true}).click();
    ...
    let text = ''; for await (const part of stream) text += part;
    expect(text).toContain('9007199254740993');
    expect(JSON.parse(text).container_version).toBe(2);
    await page.locator('input[type=file]').setInputFiles({name: 'desktop.json', mimeType: 'application/json', buffer: Buffer.from(text)});
    await expect(page.getByText('Portable', {exact: true})).toHaveCount(2);
    await page.getByRole('button', {name: 'Close', exact: true}).click();
    await advance(page, page.locator('.custom-dialogue').getByRole('button', {name: 'Continue', exact: true}));
...
for (const width of [1280, 390]) {
  test(`parallel animation, journal pages and soundtracks at ${width}`, async ({page}, info) => {
    ...
    await page.addInitScript(() => {
      const Original = window.Audio; window.testAudio = [];
      window.Audio = function(...args) { const audio = new Original(...args); window.testAudio.push(audio); return audio; };
    ...
    for (const kind of ['Frame', 'Stream']) {
      if (kind === 'Stream') {
        await page.locator('#saves').click(); page.once('dialog', dialog => dialog.accept());
      ...
      const clock = () => page.evaluate(() => window.testAudio.findLast(audio => audio.src.endsWith('/audio.wav'))?.currentTime || 0);
      await expect.poll(clock).toBeGreaterThan(.2);
      await page.locator('#saves').click();
      await expect(page.locator('#modal')).toBeVisible();
      await expect.poll(() => page.evaluate(() => window.testAudio.findLast(audio => audio.src.endsWith('/audio.wav'))?.paused)).toBe(true);
      const paused = await clock();
      await page.waitForTimeout(250); expect(Math.abs(await clock() - paused)).toBeLessThan(.05);
      await page.getByRole('button', {name: 'Save', exact: true}).nth(kind === 'Frame' ? 0 : 1).click();
      await page.locator('#close').click();
      await expect(page.locator('#text')).toContainText('Playback complete');
      await page.locator('#saves').click();
      page.once('dialog', dialog => dialog.accept());
      await page.getByRole('button', {name: 'Load', exact: true}).nth(kind === 'Frame' ? 0 : 1).click();
      await expect.poll(clock).toBeGreaterThanOrEqual(paused - .06);
      expect(await clock()).toBeLessThan(paused + .6);
      await expect(page.locator('#text')).toContainText('Playback complete');
    }
    expect(errors).toEqual([]);
  });
