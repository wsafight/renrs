...
test('partial reveal, modal pause, portable save progress and click-to-complete', async ({
  page,
}, info) => {
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      'renrs:org.renrs.signal-at-dusk:settings',
      JSON.stringify({ text_speed: 10 }),
    );
  });
  await page.goto('/');
  ...
  await expect
    .poll(async () => Number(await text.getAttribute('data-visible-characters')))
    .toBeGreaterThan(1);
  ...
  if (visible === null) throw new Error('Expected visible character progress');
  ...
  let data = '';
  for await (const part of stream) data += part;
  ...
  await expect(text).toHaveAttribute('data-revealing', 'true');
  expect(Number(await text.getAttribute('data-visible-characters'))).toBeLessThan(
    Number(visible) + 4,
  );
  await expect
    .poll(async () => Number(await text.getAttribute('data-visible-characters')))
    .toBeGreaterThan(Number(visible));
...
test('auto advance waits for reveal and its post-reveal delay', async ({ page }) => {
  await page.addInitScript(() =>
    localStorage.setItem(
      'renrs:org.renrs.signal-at-dusk:settings',
      JSON.stringify({ text_speed: 10, auto: true, auto_delay: 0.8, wait_voice: false }),
    ),
  );
  await page.goto('/');
...
test('Unicode and ruby reveal keep complete scalar values and pause in the background', async ({
  page,
}) => {
  await page.route('**/project.json', async (route) => {
    const response = await route.fetch();
    const data = (await response.json()) as { program_json: string };
    const program = JSON.parse(data.program_json) as {
      instructions: Array<{ kind: { Dialogue?: { text: string } } }>;
    };
    const dialogue = program.instructions.find((item) => item.kind.Dialogue)?.kind.Dialogue;
    if (!dialogue) throw new Error('Expected a dialogue instruction in the fixture');
    dialogue.text = 'A\u{1f600}{ruby=han}\u6c49{/ruby}Z';
