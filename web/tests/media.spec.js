...
test('video advances, freezes in saves and restores its remaining time', async({page}, testInfo) => {
  ...
  await advance(page);
  await expect.poll(() => page.locator('#background').getAttribute('src')).toContain('/clips/intro/');
  ...
  await expect.poll(() => page.getByRole('img',{name:'Rooftop',exact:true}).evaluate(image => image.naturalWidth)).toBeGreaterThan(0);
